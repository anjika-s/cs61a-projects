...
def about(keywords: list[str]):
    ...
    def checker(paragraph):
        punc = remove_punctuation(paragraph)
        lower_case = lower(punc)
        ignore = split(lower_case)
        for i in keywords:
            for k in ignore:
                if i == k:
                    return True
...
def accuracy(entered: str, source: str) -> float:
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    num_enter = len(entered_words)
    num_source = len(source_words)
    count = 0
    if num_enter == 0 and num_source == 0:
        return 100.0
    elif num_enter == 0:
        return 0.0
    elif source == 0:
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    for i in word_list:
        if i == entered_word:
            ...
            differences += [diff_function(entered_word, i, limit)]
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    if entered == source:
        ...
        counter += abs(len(entered)- len(source))
           # return counter
        if counter > limit:
            return limit + 1
        return counter
        ...
        counter += 1
        return counter + furry_fixes(entered[1:], source[1:], limit-1)
    else:
        return counter + furry_fixes(entered[1:], source[1:], limit)
    # END PROBLEM 6
...
def minimum_mewtations(entered: str, source: str, limit: int) -> int:
    ...
    if entered == '' or source == '':
        remaining = abs(len(entered) - len(source))
        if remaining > limit:
            return limit + 1
        return remaining
...
def report_progress(entered: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    for x in range(len(entered)):
        if entered[x] == source[x] and checker == True:
            correct += 1
        elif entered[x] != source[x]:
            checker = False
    ratio = correct/len(source)
    my_dict["progress"] = ratio
    upload(my_dict)
    return ratio
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    for player in ts_by_player:
        ...
        for i in range(1, len(player)):
            temp_list += [(player[i] - player[i-1])]
        times.append(temp_list)
    dict["times"] = times
    return dict
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for i in range(len(words)):
        min_num = get_time(times, 0, i)
        for j in range(len(times)):
            one = get_time(times,j,i)
            if one < min_num:
                min_num = one
                fastest[i] = j
