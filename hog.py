...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = update(strat, score0, score1, dice)
        else:
            strat = strategy1(score1, score0)
            score1 = update(strat, score1, score0, dice)
        who = 1 - who
    # END PROBLEM 5
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def player_strat(score0, score1):
        return n
    return player_strat
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score = 0
    opp_score = 0
    dice = strategy(score, opp_score)
    while score <= goal:
        while opp_score <= goal:
            next_dice = strategy(score, opp_score)
            if next_dice != dice:
                return False
            else:
                opp_score += 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    min_num_rolls = 0
    while count <= 10:
        check = make_averaged(roll_dice, times_called)(count, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    first = boar_brawl(score, opponent_score) + score
    points = sus_points(first)
    change = points - score
    if change >= threshold:
        return 0
    else:
